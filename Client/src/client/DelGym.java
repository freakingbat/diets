 ...
 * @author Bat
 */
public class DelGym extends AdminPanel {
    ...
    public void setButton(List<Identifable> content, DefaultListModel listModel, JList list, AbstractTabulatedPanel inputPanel, User user) {
        ...
        this.del.addActionListener((ActionEvent e1) -> {
            ...
            listModel.remove(id);
            try {
                Service.getDataService().deleteGymById(user.getLogin(), user.getToken(), cont.get(id).getId());
                listModel.clear();
    ...
    public List<?> setInformation(DefaultListModel listModel) {
        List<Gym> content;
        try {
            content = Service.getDataService().getGym();
