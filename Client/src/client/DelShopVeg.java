 ...
 * @author Bat
 */
public class DelShopVeg extends AdminPanel{
    ...
    public void setButton(List<Identifable> content, DefaultListModel listModel, JList list, AbstractTabulatedPanel inputPanel, User user) {
        ...
        this.del.addActionListener((ActionEvent e1) -> {
            ...
            listModel.remove(id);
            try {
                Service.getDataService().deleteShopVegById(user.getLogin(), user.getToken(), cont.get(id).getId());
                listModel.clear();
    ...
    public List<?> setInformation(DefaultListModel listModel) {
        List<ShopVeg> content;
        try {
            content = Service.getDataService().getShopVeg();
